...
def generate_dcat_json(
    ...
    logger.info(f"[generate_dcat_json] contact_point_override type: {type(contact_point_override)}")
    if isinstance(contact_point_override, dict):
        logger.info(f"[generate_dcat_json] contact_point_override hasEmail: {contact_point_override.get('hasEmail', 'MISSING')}")
    ...
    dcat_json = {
        ...
        "contactPoints": [{
            ...
            # Log what email we're using
            # Note: This comment should be removed in production
    ...
    if dcat_json.get('contactPoints') and len(dcat_json['contactPoints']) > 0:
        logger.info(f"[generate_dcat_json] Returning JSON with contactPoint hasEmail: {dcat_json['contactPoints'][0].get('hasEmail', 'MISSING')}")
